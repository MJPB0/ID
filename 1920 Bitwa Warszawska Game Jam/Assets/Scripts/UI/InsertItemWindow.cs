...
public class InsertItemWindow : MonoBehaviour
{
    ...
    public void CancelItemInserting()
    {
        ...
        if (InsertItemSlot.transform.childCount > 0)
        {
            for (int i = 0; i < inventorySpace; i++)
            {
                ...
                {
                    playerInventory.AddItem(InsertItemSlot.gameObject.GetComponentInChildren<ItemPickup>().itemInfo, i);
                    gameUI.AddItemToUIGameInventory(InsertItemSlot.gameObject.GetComponentInChildren<ItemPickup>().gameObject, i);
                    break;
                }
            }
        }
    ...
    {
        if (InsertItemSlot.GetComponentInChildren<ItemPickup>().itemInfo.ItemID == currentlyInteractingWith.ItemID)
        {
            ...
            ClearSlot();
            Destroy(InsertItemSlot.GetComponentInChildren<ItemPickup>().gameObject);
            currentlyInteractingWith = null;
        }
        else
        {
            Debug.Log("Incorrect item, canceling item insert. Expected ID: " + currentlyInteractingWith.ItemID + ", inserted item's ID: " +
                InsertItemSlot.GetComponentInChildren<ItemPickup>().itemInfo.ItemID);
            CancelItemInserting();
            currentlyInteractingWith = null;
        }
    }
